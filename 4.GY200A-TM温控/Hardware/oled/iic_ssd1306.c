 ...
 * @param[in]   y y坐标位置
 * @param[in]   num 要显示的数字
 * @param[in]   dotNum 要显示数字的小数点后几位
 */
void OLED_P8x16Num(u8 x, u8 y, float num, u8 dotNum)
{
	u8 ch[9] = { '\0', '\0', '\0', '\0', '\0', '\0','\0', '\0', '\0' };
	unsigned char c=0,i=0,j=0;
	u8 chNum = 0;
	if( num < 0 ){ ch[chNum++] = '-'; num = -num; }
	if(num > 32000) return;
	
	c = 8-chNum;
	
	if(num >= 10000){
		ch[chNum++] = num/10000 + 48; ch[chNum++] = (int)(num)%10000/1000+48; ch[chNum++] = (int)(num)%1000/100+48;  ch[chNum++] = (int)(num)%100/10+48;  ch[chNum++] = (int)(num)%10+48;
	}else if(num >= 1000){
		ch[chNum++] = (int)(num)%10000/1000+48; ch[chNum++] = (int)(num)%1000/100+48;  ch[chNum++] = (int)(num)%100/10+48;  ch[chNum++] = (int)(num)%10+48;
	}else if(num >= 100){
		ch[chNum++] = (int)(num)%1000/100+48;  ch[chNum++] = (int)(num)%100/10+48;  ch[chNum++] = (int)(num)%10+48;
	}else if(num >= 10){
		ch[chNum++] = (int)(num)%100/10+48;  ch[chNum++] = (int)(num)%10+48;
	}else if(num >= 0){
		 ch[chNum++] = (int)(num)%10+48;
	}
	if(dotNum > 0 && chNum < 7 ) {
		ch[chNum++] = '.';
		num = num - (int)num;
		
		if(dotNum == 1 && chNum < 8 ){ num = num*10;  ch[chNum++] = (int)(num+0.5)%10 + 48; }
		if(dotNum >= 2 && chNum < 8 ){  num = num*100;  ch[chNum++] = (int)num%100/10 + 48; if(chNum < 8) ch[chNum++] = (int)(num+0.5)%10 + 48; }
	}
