...
*	函数功能:	在指定坐标绘制指定颜色的点
*
*	说    明:	使用示例 LCD154_DrawPoint( 10, 10, 0x0000FF) ，在坐标(10,10)绘制蓝色的点
*
*****************************************************************************************************************************************/

void LCD154_DrawPoint(uint16_t x,uint16_t y,uint32_t color)
{
...
void LCD154_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	{
	 LCD154_DrawPoint(x,y,LCD.Color);             /* Draw the current pixel */
...
void LCD_DrawCircle(uint16_t x, uint16_t y, uint16_t r)
{
	...
	do {   
		...
		LCD154_DrawPoint(x-Xadd,y+Yadd,LCD.Color);
		LCD154_DrawPoint(x+Xadd,y+Yadd,LCD.Color);
		LCD154_DrawPoint(x+Xadd,y-Yadd,LCD.Color);
		LCD154_DrawPoint(x-Xadd,y-Yadd,LCD.Color);
  ...
  { 
    do {
      K = (float)(rad1/rad2);
		...
		LCD154_DrawPoint(x-Xadd,y+(uint16_t)(Yadd/K),LCD.Color);
		LCD154_DrawPoint(x+Xadd,y+(uint16_t)(Yadd/K),LCD.Color);
		LCD154_DrawPoint(x+Xadd,y-(uint16_t)(Yadd/K),LCD.Color);
		LCD154_DrawPoint(x-Xadd,y-(uint16_t)(Yadd/K),LCD.Color);     
  ...
  else
  {
    ...
    do { 
      K = (float)(rad2/rad1);
		...
		LCD154_DrawPoint(x-(uint16_t)(Xadd/K),y+Yadd,LCD.Color);
		LCD154_DrawPoint(x+(uint16_t)(Xadd/K),y+Yadd,LCD.Color);
		LCD154_DrawPoint(x+(uint16_t)(Xadd/K),y-Yadd,LCD.Color);
		LCD154_DrawPoint(x-(uint16_t)(Xadd/K),y-Yadd,LCD.Color);  
