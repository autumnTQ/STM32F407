...
//>>>>>	2D图形函数
void  LCD154_DrawPoint(uint16_t x,uint16_t y,uint32_t color);   	//画点
...
void  LCD154_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2);	// 两点之间画线
...
#define 	LCD_SPI                         SPI2				// LCD用到的是SPI2
...
#define 	GPIO_LCD_SCK_PinSource       	GPIO_PinSource13
#define 	GPIO_LCD_SCK_AF              	GPIO_AF_SPI2
...
#define 	GPIO_LCD_SDA_PinSource       	GPIO_PinSource15
#define 	GPIO_LCD_SDA_AF              	GPIO_AF_SPI2

#define 	LCD_CS_PIN       				GPIO_Pin_12						// CS片选引脚，低电平有效
#define 	LCD_CS_PORT      				GPIOB                 			// CS引脚用到的端口 
#define 	GPIO_LCD_CS_CLK     			RCC_AHB1Periph_GPIOB				// CS引脚IO口时钟

#define     LCD_DC_PIN						GPIO_Pin_14				        // 数据指令选择  引脚				
#define	    LCD_DC_PORT						GPIOB							// 数据指令选择  GPIO端口
#define 	GPIO_LCD_DC_CLK     			RCC_AHB1Periph_GPIOB			// 数据指令选择  GPIO时钟 	
